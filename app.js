...
const statsCompletedQuizzesTrend = document.getElementById(
  "statsCompletedQuizzesTrend"
);
const statsAverageScoreTrend = document.getElementById(
  "statsAverageScoreTrend"
);
...
const scoreDistributionChart = document.getElementById(
...
function startTimer() {
  ...
  timerDisplay.innerHTML = `<i class="far fa-clock"></i> ${formatTime(
...
function loadProgress(quiz) {
  ...
  if (savedProgress) {
    ...
    shuffledQuestions = progress.shuffledQuestions.map(
      (id) => quiz.questions.find((q) => q.id === id) || quiz.questions[0]
    );
    return true;
  }
  return false;
    ...
    if (resetButton) {
      resetButton.onclick = async (e) => {
        ...
        const confirmed = await showCustomConfirm(
          "Bạn có chắc chắn muốn xóa tiến trình của bài kiểm tra này?"
        );
        if (confirmed) {
...
function displayQuestion() {
  ...
  quizProgressText.textContent = `Câu ${currentQuestionIndex + 1} / ${
  ...
  questionContainer.innerHTML = `
        <div class="space-y-4">
            <h3 class="text-xl font-medium question-text mb-4">${
              question.question
            }</h3>
            <div class="space-y-3">
                ${question.options
                  .map(
                    ...
                    <div class="option-card ${
                      userAnswers[currentQuestionIndex] === index
                        ? "selected"
                        : ""
                    }" data-index="${index}" tabindex="0" role="button" aria-pressed="${
                      userAnswers[currentQuestionIndex] === index
                    }">
...
resetProgressButton.onclick = async () => {
  const confirmed = await showCustomConfirm(
    "Bạn có chắc chắn muốn xóa tiến trình bài trắc nghiệm này?"
  );
  if (confirmed) {
    stopTimer();
...
function showResult() {
  ...
  const scoreInterval = setInterval(() => {
    ...
    scoreDisplay.textContent = `Điểm: ${displayScore.toFixed(
      1
    )}/10 (${correctAnswersCount}/${totalQuestions} câu đúng)`;
  }, 20);
  ...
  timeTakenDisplay.textContent = `Thời gian hoàn thành: ${formatTime(
    timeElapsed
  )}`;
  ...
  userAnswers.forEach((answer, index) => {
    ...
    if (answer !== question.correctAnswer && answer !== null) {
      ...
      wrongDiv.innerHTML = `
                <h4 class="wrong-answer-question">Câu ${index + 1}: ${
        question.question
      }</h4>
                <div class="wrong-answer-user">
                    <i class="fas fa-times-circle mr-2"></i> Bạn chọn: ${
                      question.options[answer]
                    }
                </div>
                <div class="wrong-answer-correct">
                    <i class="fas fa-check-circle mr-2"></i> Đáp án đúng: ${
  ...
  if (wrongAnswersDisplay.innerHTML === "") {
    wrongAnswersDisplay.innerHTML =
      '<div class="p-6 bg-dark-50 dark:bg-dark-900/30 text-dark-600 dark:text-dark-400 rounded-lg text-center slide-in"><i class="fas fa-trophy text-4xl mb-3"></i><p class="text-xl font-semibold">Chúc mừng! Bạn trả lời đúng tất cả các câu.</p></div>';
...
restartButton.onclick = () => {
  ...
  startQuiz(
    currentQuiz,
    shuffledQuestions.length !== currentQuiz.questions.length
  );
};
...
function updateStats() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    if (dayResults.length > 0) {
      const dayScore =
        dayResults.reduce((sum, item) => sum + item.score, 0) /
        dayResults.length;
      const dayCorrect = dayResults.reduce(
        (sum, item) => sum + item.correctAnswers,
        0
      );
      const dayTotal = dayResults.reduce(
        (sum, item) => sum + item.totalQuestions,
        0
      );
...
function updateRecentQuizTable() {
  ...
  recentQuizzes.forEach((quiz) => {
    ...
    row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900 dark:text-white">${quiz.score.toFixed(
                  1
                )}/10</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-500 dark:text-gray-400">${formatTime(
                  quiz.timeSpent
                )}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
...
function updateQuizHistoryTable() {
  ...
  sortedHistory.forEach((quiz) => {
    ...
    row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900 dark:text-white">${
                  quiz.correctAnswers
                }/${quiz.totalQuestions}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3" data-quiz-id="${
                  quiz.quizId
                }">
                    <i class="fas fa-redo"></i> Làm lại
                </button>
                <button class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300" data-result-id="${
                  quiz.id
                }">
    ...
    const retryButton = row.querySelector(
      `button[data-quiz-id="${quiz.quizId}"]`
    );
...
function updateTimelineChart() {
  ...
  const correctRates = sortedHistory.map(
    (item) => (item.correctAnswers / item.totalQuestions) * 100
  );
...
function updateTimeSpentChart() {
  ...
  const timeSpent = sortedHistory.map((item) =>
